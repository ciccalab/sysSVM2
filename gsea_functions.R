...
do_GSEA = function(syscan, pathway_df, geneType = "prediction", topN = 10, fdr_threshold = 0.01, pathway_levelLimits = c(2, NA), exclude_pathways = NULL, pathway_sizeLimits = c(10, 500)){
  ...
  syscan_gsea = syscan_gsea %>% arrange(sample, desc(score)) %>% group_by(sample) %>% mutate(patient.rank = 1:n()) %>% ungroup
  if (!is.null(topN)) syscan_gsea = syscan_gsea %>% subset(patient.rank <= topN)
